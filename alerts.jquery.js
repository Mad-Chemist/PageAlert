var alertNumber =0, alertWidth = 300, alertDelay = 4000;
$(document).ready(function() {
	$('body').append('<div id="pageAlerts"></div>');
	$("<style type='text/css'>#pageAlerts { position:fixed; top:0; width:100%; z-index:9999999999999; pointer-events:none;} #pageAlerts .alert {width: "+alertWidth+"px; margin: 5px auto; color: #FFF; display: block; padding: 10px; text-align: center; font-weight: bold; z-index:99999999999999;} #pageAlerts .alert.good { background:green; } #pageAlerts .alert.bad { background:#E00000;;} .round6 { -webkit-border-radius: 6px; -moz-border-radius: 6px; border-radius: 6px; }</style>").appendTo("head");
});
...
// $(WHERE YOU WANT ALERT TO SHOW -> for best results this should be '#pageAlerts').pageAlert('TEXT YOU WANT SHOWN', 1 for red and 0 for green background)
